...
def avctp_test(psm):
    try:
        sock = bluetooth.BluetoothSocket(bluetooth.L2CAP)
        sock.settimeout(10)
        sock.connect((bdaddr, 0x19))
        time.sleep(0.1)
        sock.connect((bdaddr, psm))
        print(Fore.GREEN + f'AVCTP:{psm}  Connect Success' + Fore.RESET)
        time.sleep(0.1)
        sock.close()
    except Exception:
        print(Fore.RED + 'No AVCTP Protocol')
...
def pbap_test(prot):
    try:
        self_port = search_port(bdaddr=bdaddr, types=prot)
...
if sys.argv[1] == 'SDP':
    ...
    rfcomm_test(rfcomm_psm)
elif sys.argv[1] == 'BNEP':
    bnep_test(bnep_psm)
elif sys.argv[1] == 'AVRCP':
    avrcp_test(avrcp_psm)
elif sys.argv[1] == 'AVDTP':
    avdtp_test(avdtp_psm)
elif sys.argv[1] == 'AVCTP':
    avctp_test(avctp_psm)
