...
def main():
    use_helper = f"""wireshark [options] ... [ <infile> ]
...
Capture interface:
    ...
    if args.fun.startswith('le_scan'):
        le_scan()
    if args.fun.startswith('check'):
        if not args.cod:
            parser.error(f'sudo python3 {current_file} -f check -c 0x200404')
        else:
            show_class_of_device(args.cod)
    if args.fun.startswith('search'):
        if not args.mac:
            parser.error(f'sudo python3 {current_file} -f search -m 3C:28:6D:29:5A:A0')
        else:
            bluetooth_services_and_protocols_search(args.mac)
    if args.fun.startswith('fuzz'):
        if not args.mac:
            parser.error(f'sudo python3 {current_file} -f fuzz -m 22:22:C5:21:D5:B3')
        if args.mac:
